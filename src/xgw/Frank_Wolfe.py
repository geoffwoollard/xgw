...
def line_search_DGW_3d(sigma_1, sigma_0):
    ...
    if DELT>0:
        ...
        q = alpha*lbd**3 + beta*lbd**2 + gamma*lbd + delta
        if 0<lbd and lbd<1 and q>d_1 and q>d_0:
            tau = lbd
            A = False
    if A:
        if d_1>d_0:
    ...
    T = alpha*tau**3 + beta*tau**2 + gamma*tau + delta
    return 6*T,tau



def line_search(sigma_1, sigma_0, cost='IGW'):
    if cost == 'IGW':
...
def Frank_Wolfe(mu, space_x, nu, space_y, cost='IGW', pi_n=None):
    if pi_n is None:
        pi_n = np.outer(mu, nu)
    n_iters = 50
