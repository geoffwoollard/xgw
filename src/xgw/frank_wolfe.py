...
def matrix_cofactor_low_dim(mat):
    ...
    for row in range(row_l):
        for col in range(col_l):
            mask_col = np.ones(row_l, dtype=np.bool_)
            mask_col[col] = False
            mask_row = np.ones(row_l, dtype=np.bool_)
            mask_row[row] = False
...
def optimize_deg_2_polynomial(alpha, beta, gamma):
    # Add tolerance if tau is close to 0 or one
    def q(x):
...
def dgw_3d_polynomial(sigma_1, sigma_0):
    ...
    TR_1 = 1/2*((tr_1**2-tr_11)*tr_0) + tr_110 -tr_1*tr_10
    TR_0 = 1/2*((tr_0**2-tr_00)*tr_1) + tr_001 -tr_0*tr_10
...
def center_marginal(mu, space_x, nu, space_y):
    space_x -= (space_x * mu[:, None]).sum(axis=0)
    space_y -= (space_y * nu[:, None]).sum(axis=0)
    return space_x, space_y


# def _frank_wolfe_iter(mu, space_x, nu, space_y, pi_n, R, e_base, cost='IGW', iter_max=50, t=0.5):
#     d = space_x.shape[-1]
...
#             init_direc = projection(pi_n, e_base)
...
def _frank_wolfe_iter(mu, space_x, nu, space_y, cost='IGW', pi_n=None, iter_max = 50, t=0.5, emd_kwargs = {}):
    if pi_n is None:
        pi_n = np.outer(mu, nu)
    ...
    for it in range(iter_max):
        ...
        pi_n_1 = tau*pi_n_1_hat + (1-tau)*pi_n
        pi_n = pi_n_1
        yield initial_cost-2*T, pi_n, initial_cost-2*initial_coupling_cost
