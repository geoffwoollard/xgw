...
def testing_2d_convex():
    ...
    for test_id in range(n_tests):
        ...
        for p_plus_implementation in ['cdd', 'h_to_v_edges']:
            T, plan, c, _, _ = gw_m_convex(mus, space_xs, mus, space_xs, {}, cost='IGW', cost_tol=1e-15, iter_max=200, p_plus_implementation=p_plus_implementation) 
            plan_error = np.linalg.norm(plan - np.eye(len(mus))/len(mus))
            print(f'Plan error for identical marginals (IGW, convex, {p_plus_implementation}): ', plan_error)
            assert np.isclose(plan_error, 0.0)
