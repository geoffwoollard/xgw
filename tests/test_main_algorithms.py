...
def test_2d_classical_gw():
    ...
    for test_id in range(n_tests):
        ...
        _, plan, _, _, __, _ = classical_gw(mus, space_xs, mus, space_xs, {},  cost_tol=1e-8, iter_max=50) 
        ...
        _, plan, _, _, _, _, _ = classical_gw(mus, space_xs, mus, space_xs@M, {},  cost_tol=1e-8, iter_max=50) 
        plan_error = np.linalg.norm(plan - np.eye(len(mus))/len(mus))
        logger.info(f'Plan error for identical marginals (classic GW, convex):  {plan_error}')
        assert np.isclose(plan_error, 0.0)
    ...
    for d in [2]:
        for test_id in range(n_tests):
            for p_plus_implementation in p_plus_implementations_to_test:
                ...
                cost = 'IGW' 
                T, plan, gap, _, _, _, _ = gw_m_convex(mus, space_xs, mus, space_xs_reflected, {}, cost=cost, gap_tol=1e-5, iter_max=50, p_plus_implementation=p_plus_implementation)
                ...
                _, plan, _, _, _, _, _ = gw_m_convex(mus, space_xs, mus, space_xs_transformed, {}, cost=cost, gap_tol=1e-5, iter_max=50, p_plus_implementation=p_plus_implementation)
                mis_match = np.isclose(plan, np.eye(len(mus))/len(mus), atol=1e-7/len(mus)) == False
                logger.info(f'Plan (should be id): {plan}')
                assert np.isclose(mis_match.sum(), 0.0), "Reflection test failed!"
    ...
    for d in [2]:
        for test_id in range(n_tests):
            ...
            for p_plus_implementation in p_plus_implementations_to_test:
                ...
                cost = 'IGW'
                T, plan, gap, _, _, _, _ = gw_m_convex(mus, space_xs, mus, space_xs_rotated, {}, cost=cost, gap_tol=1e-5, iter_max=50, p_plus_implementation=p_plus_implementation)
...
def test_cgw_convex_rotation_invariant():
    ...
    for d in [2]:
        for test_id in range(n_tests):
            ...
            logger.info(f'Rotation matrix: {rotation.shape}')
            space_xs_rotated = space_xs @  rotation.T
            _, plan, _, _, _, _, _ = gw_m_convex(mus, space_xs, mus, space_xs_rotated, {}, cost='CGW', gap_tol=1e-5, iter_max=50)
            mis_match = np.isclose(plan, np.eye(len(mus))/len(mus), atol=1e-7/len(mus)) == False
...
def test_2d_convex(p_plus_implementations_to_test, p_minus_implementations_to_test, p_minus_dual_implementations_to_test):
            ...
            for p_plus_implementation in p_plus_implementations_to_test:
                for p_minus_implementation in p_minus_implementations_to_test:
                    for p_minus_dual_implementation in p_minus_dual_implementations_to_test if p_minus_implementation == 'v_to_h_dual' else [None]:
                        ...
                        T, plan, c, _, _, _, _ = gw_m_convex(mus, space_xs, mus, space_xs, {}, cost=cost, gap_tol=1e-15, iter_max=200, p_plus_implementation=p_plus_implementation, p_minus_implementation=p_minus_implementation, p_minus_dual_implementation=p_minus_dual_implementation) 
                        plan_error = np.linalg.norm(plan - np.eye(len(mus))/len(mus))
    ...
    for cost in ['IGW', 'CGW']:
        for p_plus_implementation in p_plus_implementations_to_test: 
            ...
            T, _, gap, _, _, _, _ = gw_m_convex(mu, space_x, mu, space_x, {}, cost=cost, gap_tol=near_zero_tolerance, iter_max=200, FW_iter=200, p_plus_implementation=p_plus_implementation)
            ...
            T, _, gap, _, _, _, _ = gw_m_convex(mu, space_x, nu, space_y, {}, cost=cost, gap_tol=near_zero_tolerance, iter_max=80, p_plus_implementation=p_plus_implementation) 
            ...
            T, _, gap, _, _, _, _ = gw_m_convex(mus, space_xs, nus, space_ys, {}, cost=cost, gap_tol=near_zero_tolerance, iter_max=200, p_plus_implementation=p_plus_implementation) 
...
def test_3d_convex(p_plus_implementations_to_test, p_minus_implementations_to_test, p_minus_dual_implementations_to_test, iter_max=20):
    '''passing'''
            ...
            for p_plus_implementation in p_plus_implementations_to_test:
                for p_minus_implementation in p_minus_implementations_to_test:
                    for p_minus_dual_implementation in p_minus_dual_implementations_to_test if p_minus_implementation == 'v_to_h_dual' else [None]:
                        ...
                        total_loss, pi_opt, gap, lower_bound_on_total_loss, cst_cost, _, _ = gw_m_convex(mus, space_xs, mus, space_xs, {}, cost=cost, gap_tol=1e-15, iter_max=iter_max, t=t_use, p_plus_implementation=p_plus_implementation, FW_iter=500, p_minus_implementation=p_minus_implementation, p_minus_dual_implementation=p_minus_dual_implementation) 
                        ...
                        assert np.isclose(plan_error, 0.0), f"Plan error {plan_error} is not close to 0 for identical marginals ({cost}, convex, {p_plus_implementation}) in test {test_id}"
                        assert np.isclose(total_loss, 0.0, atol=1e-5), f"Total loss {total_loss} is not close to 0 for identical marginals ({cost}, convex, p_plus {p_plus_implementation}), p_minus {p_minus_implementation}, p_minus_dual {p_minus_dual_implementation} in test {test_id}"

                        logger.info(f'Test {test_id}, cost: {cost}, t_use: {t_use}, p_plus_implementation: {p_plus_implementation} - different simple marginals')
                        total_loss, pi_opt, gap, lower_bound_on_total_loss, cst_cost, _, _ = gw_m_convex(mus, space_xs, nus, space_ys, {}, cost=cost, gap_tol=1e-15, iter_max=iter_max, t=t_use, p_plus_implementation=p_plus_implementation, FW_iter=500, p_minus_implementation=p_minus_implementation, p_minus_dual_implementation=p_minus_dual_implementation) 
                        logger.info(f'Total loss: {total_loss:.6f}, lower bound: {lower_bound_on_total_loss:.6f}, gap: {gap:.6f}, constant cost: {cst_cost:.6f}')
