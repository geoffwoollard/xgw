...
from xgw.gromov_wasserstein_m_dist import _run_convex_yield,  _run_classical_yield
...
def bounding_box_convergence_rate(n_points, niter, d):
    ...
    if not os.path.exists('experiments/figures'):
        os.makedirs('experiments/figures')
    fig.suptitle(f'{n_points} points in marginals, dimension {d}' )
    plt.tight_layout()
...
def convex_cost_convergence_rate(n_points, niter, d, cost, ryner_direc=False):
    ...
    for elem in tqdm(_run_convex_yield(mu, space_x, nu, space_y, emd_kwargs={}, niter = niter, cost=cost, p_plus_implementation= 'h_to_v_popcount',ryner_direc=ryner_direc)):
    ...
    plt.tight_layout()
    fig.savefig(f'experiments/figures/{cost}_cost_convergence_d{d}_{n_points}points_grad_dir{ryner_direc}_{niter}iter.svg', format='svg')
    plt.close(fig)  
    

def classical_gw_convergence_rate(n_points, niter, d, ryner_direc=False):
    ...
    for elem in tqdm(_run_classical_yield(mu, space_x, nu, space_y, emd_kwargs={}, niter = niter, p_plus_implementation='h_to_v_popcount',  ryner_direc=ryner_direc)):
        P_plus, P_minus,  c_plus, c_minus = elem
    ...
    for idx in range(n_panels):
        ...
        if idx ==0:
            ...
            axes[idx].set_ylabel(f'Bound on value \n '+r'of the GW$_2$ distance')
            axes[idx].set_yscale('log')
        axes[idx].legend()
    # mkdir if not exists
    if not os.path.exists('experiments/figures'):
        os.makedirs('experiments/figures')
    fig.suptitle(f' {n_points} points in marginals, dimension {d}, certificate {objective_list[-1]}' )
    plt.tight_layout()
    fig.savefig(f'experiments/figures/2-GW_cost_convergence_d{d}_{n_points}points_grad_dir{ryner_direc}_{niter}iter.svg', format='svg')
    plt.close(fig)  
...
if __name__ == "__main__":
    ...
    # convex_cost_convergence_rate(2000, 200, 2,'IGW',ryner_direc=False)
    # convex_cost_convergence_rate(2000, 200, 2,'IGW',ryner_direc=True)
    convex_cost_convergence_rate(200, 400, 2,'IGW',ryner_direc=False)
    convex_cost_convergence_rate(2000, 100, 2,'IGW',ryner_direc=True)
    convex_cost_convergence_rate(2000, 100, 2,'IGW',ryner_direc=False)
    convex_cost_convergence_rate(2000, 300, 2,'IGW',ryner_direc=True)
    convex_cost_convergence_rate(2000, 300, 2,'IGW',ryner_direc=False)
    ...
    # classical_gw_convergence_rate(300, 300, 2,ryner_direc=True)
    # classical_gw_convergence_rate(300, 300, 2,ryner_direc=False)
    # classical_gw_convergence_rate(10, 35, 3,ryner_direc=False)
    # classical_gw_convergence_rate(10, 35, 3,ryner_direc=True)